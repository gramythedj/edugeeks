...
{
    return Tea.encrypt(msg, QUIZ_KEYWORD);
}
...
{
    return Tea.decrypt(msg, QUIZ_KEYWORD);
}

// -----------------------------------
// Decodifica un numero
// -----------------------------------
// La funzione genera un errore se il numero convertito non e' nei limiti
// massimi aspettati (0,maxNum) in quanto la keyword e' sbagliata.

function DecodeNumber(msg,n,minNum,maxNum)
{
	// msg = numero (risposta esatta) da decifrare
	// n = numero di domanda
	// minNum = valore minimo aspettato
	// maxNum = valore massimo aspettato
	var clearNumber;
	var plainText;

    plainText = Tea.decrypt(msg, QUIZ_KEYWORD);
	if (plainText.length > 1) 
	{
        if (plainText.startsWith('NANS')) {
...
function CodeNumber(clearNumber, n) 
{
    ...
    plainText = "NANS" + num.toString();
    return Tea.encrypt(plainText, QUIZ_KEYWORD);
}
