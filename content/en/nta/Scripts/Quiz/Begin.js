  ...
  $(".test-questions")
    ...
    .each(function () {
      ...
      var currectQue = $("#" + a)
        .find(".question-title")
        .text();
      var CorrectAnsMarks = $("#" + a)
        .find(".hdfCorrectAnsMarks")
        .val();
      var InCorrectAnsMarks = $("#" + a)
        .find(".hdfInCorrectAnsMarks")
        .val();
      ...
      if (
        ...
        if (ansStatus == "Correct") {
          score = score + parseInt(CorrectAnsMarks);
          TotalCorrect = TotalCorrect + 1;
        } else {
          score = score + parseInt(InCorrectAnsMarks);
      ...
      if (
        ...
        if (ansStatus == "Correct") {
          ...
          tr.append(
            '<td><span class="label label-danger">' + ansStatus + "</span></td>"
          );
        }
      } else {
        tr.append("<td>N/A</td>");
      ...
      //$('#tbodyResult').dataTable({ paging: true });
    });
...
$(document).ready(function () {
  //alert(PaperQuestions);
  if ($("#ExamID").val() == "14") {
    $("#drplanguage").append(
      $("<option></option>").text("Gujarati").val("gujarati")
    );
  }
  $("#page01").show();
  $(".exam-paper").show();
  ...
  $(".test-ques").click(function () {
    ...
    $(".test-questions").find("li").removeClass("active"),
      ...
      e.hasClass("que-save") ||
        ...
        e.hasClass("que-mark") ||
        (e.addClass("que-not-answered"), e.removeClass("que-not-attempted")),
      OpenCurrentQue($(this));
  });
        ...
        .each(function () {
          ...
          if (
            (e.children().hasClass("que-save")
              ? (t = !0)
              : e.children().hasClass("que-save-mark") && (t = !0),
            t)
          ) {
  ...
  $(".drplanguage").on("change", function (e) {
    ...
    //alert($('.question-height > .question-title').val());
    //if ($(this).val() == 'english') {
    //    newlang = 'English';
    ...
    $(".question-height > .MQ").each(function (index, item) {
      var currentImg = $(this);
      var currentQuestionNo = currentImg.data("questionno");
      var currentDisImg = $(".question-height > ." + currentQuestionNo + "DQ");

      $.each(JSON.parse(PaperQuestions), function (i, item) {
        if (
          item.QuestionNo == currentQuestionNo &&
          item.LangID == parseInt(newlang)
        ) {
          currentDisImg.attr(
            "src",
            $("#datalink").val() + item.DescriptionImagePath
          );
  ...
  $(".stream_1").on("click", function (e) {
    ...
    var a = $(".test-questions")
      .find("li")
      .find("a[data-href=" + current_herf + "]");
    a.trigger("click");
  });
